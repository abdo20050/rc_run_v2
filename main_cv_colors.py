...
def main():
    ...
    lab_count = 0
    start_counter = False
    num_points_avg = 30 # How many points from the trace to avg
    steerLock_duration = 0
    stuck_checker_obj = stuck_checker()
    steer_angle = 0
    steer_angle_buffer = collections.deque(maxlen=20) #Increase this for smoother steering but too much will cause some steer delay
    run_speed = 0.8
    try:
        while True:
            curr_time = time.time()
            ...
            if stuck_checker_obj.check_if_stuck(frame):
                steerLock_duration = 2 #seconds to go backward
